...
class OperationResult:
    """Data class representing the result of a camera operation."""
...
def start_health_checks():
    ...
    def publish_periodically():
        while True:
            ...
            mqtt_client.publish(
                "te/device/main/service/vision-ai-mapper/status/health",
                payload=msg,
                retain=True,
            )
            time.sleep(30)
...
def on_message(client, userdata, msg):
    ...
    if te_match := re.match(r"te/device/([^/+]+)///cmd/([^/+]+)/([^/+]+)", msg.topic):
        ...
        if payload["status"] == "init":
            ...
            if command == "activate_model":
                ...
                mqtt_client.publish(
                    f"vai/{camera_id}/model/activate/{op_id}", model_name, retain=True
                )
            elif command == "image_capture":
                mqtt_client.publish(
                    f"vai/{camera_id}/image/{op_id}",
                    json.dumps(payload["c8y_ImageCapture"]),
                    retain=True,
                )
    ...
    # handle image results
        ...
        try:
            ...
            log.error(e)
            handle_image_error(
                op_id, camera_id, "Could not send or parse image result message"
            )

    # handle model activation results
    elif model_result_match := re.match(
        r"vai/([^/]+)/model/activate/([^/]+)/result", msg.topic
    ):
        ...
        try:
            ...
            if operation_result.status == "successful":
                ...
                handle_activate_model_error(
                    op_id, camera_id, operation_result.failure_reason
                )
        except Exception as e:
            log.error(e)
            handle_activate_model_error(
                op_id, camera_id, "Could not send or parse model activation message"
            )
...
def handle_image_success(opid, camera_id, filename):
    ...
    media_info = {"c8y_CameraOutput": {"type": "image", "format": "jpg"}}
    subprocess.run(
        [
            "tedge",
            "upload",
    ...
    os.remove(filename)
    mqtt_client.publish(
        f"te/device/{camera_id}///cmd/image_capture/{opid}", json.dumps(original_op)
    )
...
def handle_activate_model_success(opid, camera_id, model_name):
    """Handle successful model activation operation."""
    original_op = operations_in_progress[opid]
    original_op["status"] = "successful"
    mqtt_client.publish(
        f"te/device/{camera_id}///cmd/activate_model/{opid}", json.dumps(original_op)
    )


def handle_activate_model_error(opid, camera_id, failure_reason):
    """Handle failed model activation operation."""
    original_op = operations_in_progress[opid]
    original_op["status"] = "failed"
    mqtt_client.publish(
        f"te/device/{camera_id}///cmd/activate_model/{opid}", json.dumps(original_op)
...
def handle_video_success(opid, camera_id, filename):
    ...
    original_op["status"] = "successful"
    device_id = original_op["externalSource"]["externalId"]
    media_info = {"c8y_CameraOutput": {"type": "video", "format": "mp4"}}
    subprocess.run(
        [
            "tedge",
            "upload",
